 ...
 * @ Modified time: 2024-03-13 10:03:48
 */
...
public class Main {
    public static void main(String[] args) throws InterruptedException {
        ...
        if (args.length > 0) {
            for (String arg: args) {
                String fileName = arg;
                try {
                    Scanner file_sc = new Scanner(new File(fileName));
                    while (file_sc.hasNextLine()) {
                        String line = file_sc.nextLine();
                        stop = handleUserInput(line, flightManager, stop);
                    }
                    file_sc.close();
            ...
            while (!stop) {
                ...
                stop = handleUserInput(userInput, flightManager, stop);
            }
            input_sc.close();
        }
    ...
    private static boolean handleUserInput(String userInput, FlightManager flightManager, boolean stop) {
